...
public class PmgetDao extends Dao {
	...
	public void insert(PmgetBean pgb, PmBean pb) throws NamingException, SQLException {
		// 副問い合わせ:pmname,pmpassからpmidを取得
		String sql1 = "(SELECT pmid FROM pmaster WHERE pmname=? AND pmpass=?)";
		// 主問い合わせ:INSERT文
		String sql = "INSERT INTO pmget" + "(pmid,pid,nickname,gen)" + "VALUES(" + sql1 + ",?,?,?)";
	...
	public List<PmgetBean> select(PmBean pb) throws NamingException, SQLException {
		String sql1 = "(SELECT pmid FROM pmaster WHERE pmname=? AND PMPASS=?)";
		String sql = "SELECT * FROM pmget pm " + "JOIN pokemon p ON pm.pid = p.pid "
				+ "JOIN ptype pt ON p.ptid = pt.ptid " + "WHERE pmid = " + sql1;
		PreparedStatement stmt = prepareStatement(sql);
		stmt.setString(1, pb.getPmname());
		stmt.setString(2, pb.getPmpass());
